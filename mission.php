<?php
	require "includes/header.php";
?>
<!-- Mission Jumbotron -->
<div class="secs">
    <div class="sec1">
        <div class="jumbotron jumbotron-fluid">
            <div class="container state">
                <h1 class="display-4 mtit">Our Mission: Advancement</h1>
                <p class="lead mstmnt">Here at Cybergence, we believe that humanity develops progress.
                    With this evident truth, we cultivate a sense of open-mindedness and tolerance for ideas that differ
                    from our own.
                    Our goal is to pave a path of innovation and ingenuity within the cybernetic industry,
                    improving quality of life, ultimately contributing to the greater-good of the world.</p>
            </div>
        </div>
        <!-- End of Mission Jumbotron -->
        <!-- Start of Explanation & gif-->
        <div class="row">
            <div class="col-6">
                <img class="misquotegif" src="img/misquote.gif" alt="tech earth">
            </div>
            <div class="col-6">
                <h2 class="lead misquote">"The idea that we are at the forefront of advancement, and that cybergence
                    exhibits our pure human
                    nature of always wanting to improve is absolutely astonishing to me. Waking up and realizing that
                    we are creating products and tools that neither my grandparents nor even my parents
                    could not have dreamt of is an incredible reality we live in." - Colin Richardson
                </h2>
            </div>
        </div>
    </div>
</div>
<!-- End of Explanation & gif-->
<!-- Ethics Jumbotron -->
<div class="row fadein">
    <div class="col-12 ethicback sec2">
        <div class="jumbotron jumbotron-fluid">
            <div class="container state">
                <h1 class="display-4 ethtit">Our Ethics: Morality-based</h1>
                <p class="lead ethdesc">There exists a debate regarding the ethics and morals surrounding cybernetics.
                    Concerns including privacy, harm, theft, and much more are addressed in the following bullet
                    points:</p>
            </div>
        </div>
        <!-- End of Ethics Jumbotron -->
        <!-- Start of Ethics List-->
        <ul class="lead ethbul">
            <li>We maintain secular standards, calling our employees to not uses biases stemmed from religious
                beliefs.</li>
            <br>
            <li>We implement guidelines that refrain from tracking.</li>
            <br>
            <li>We consult experts in purity and fair-based morals to ensure we have the best intentions in mind.
            </li>
            <br>
            <li>We conduct routine investigations to make sure no information is being stolen.</li>
            <br>
            <li>We run analyses on all of our employees every month to guarantee works that contribute to the
                greater-good of the world.</li>
        </ul>
    </div>
</div>
<!-- End of Ethics List-->
<!-- Team-->
<div class="row fadein secs">
    <div class="col-12 teamback sec3">
        <div class="jumbotron jumbotron-fluid">
            <div class="container state">
                <h1 class="display-4 expteam">Our Team: Experienced</h1>
                <p class="lead expdesc">Our team is the very personification of our core values: advancement,
                    innovation,
                    and
                    growth. Each employee is a pioneer in their field, furthering human progress towards
                    cybernetics. We
                    believe our ensemble of trailblazers will progress Cybergence to new heights.
                </p>
            </div>
        </div>
        <!-- Start of Team Cards-->
        <div class="team-container">
            <div class="team-item">
                <div class="team-card">
                    <div class="team-card-image">
                        <img src="img/colin.jpg" alt="Colin">
                    </div>
                    <div class="team-card-title">
                        <h2>Colin Richardson</h2>
                    </div>
                    <div class="team-card-description">
                        <p>Colin Richardson is our CEO. He graduated from Georgetown University and
                            currently resides in Boston
                            with his wife and 3 boys.
                            Colin's favorite pastime is playing soccer with his children.
                            "I yearn for Cybergence to allow one to express their creative endeavors while also
                            working in a professional culture of progress"</p>
                    </div>
                </div>
            </div>
            <div class="team-item">
                <div class="team-card">
                    <div class="team-card-image">
                        <img src="img/matt.png" alt="Matt">
                    </div>
                    <div class="team-card-title">
                        <h2>Matthew Thornton</h2>
                    </div>
                    <div class="team-card-description">
                        <p>Matthew Thornton is our cyber-robotic engineer. He graduated from Georgia Tech and
                            currently resides in Atlanta with his wife and two daughters. Matt's favorite pastime is
                            repairing computers and testing out different cooking recipes.
                            "I am very happy to be apart of this team since it connects my two passions: tech and
                            people."</p>
                    </div>
                </div>
            </div>
            <div class="team-item">
                <div class="team-card">
                    <div class="team-card-image">
                        <img src="img/vincent.png" alt="Vincent">
                    </div>
                    <div class="team-card-title">
                        <h2>Vincent Zhu</h2>
                    </div>
                    <div class="team-card-description">
                        <p>Vincent Zhu is our intern. He graduated from Jackson State Community College and
                            curently resides in Alaska, alone. Vincent's favorite pastime is fishing. "Being an intern
                            is a truly unique role that suits me best."</p>
                    </div>
                </div>
            </div>
        </div>
        <!-- End of Team Cards-->
    </div>
</div>
<!-- End of Team -->
<?php
	require "includes/misfooter.php";
?>
<script src="js/scripts.js"></script>
</body>

</html>